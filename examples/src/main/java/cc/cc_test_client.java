...
final class cc_test_client {
  public static void main(final String args[]) throws Exception {
    ...
    final String dest_host = args[2];
    final int dest_port = Integer.parseInt(args[3]);
    ...
    //  { CC-Request-Type }
    ccr.add(
        new AVP_Unsigned32(ProtocolConstants.DI_CC_REQUEST_TYPE, ProtocolConstants.DI_CC_REQUEST_TYPE_EVENT_REQUEST));
    ;
    //  { CC-Request-Number }
    ...
    final AVP result_code = cca.find(ProtocolConstants.DI_RESULT_CODE);
    if (result_code == null) {
      System.out.println("No result code");
      return;
    }
    try {
      final AVP_Unsigned32 result_code_u32 = new AVP_Unsigned32(result_code);
      final int rc = result_code_u32.queryValue();
      switch (rc) {
