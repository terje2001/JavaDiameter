...
/**
 * ABNFConverter - convert RFC-style ABNF to java class. This program takes as input (input redirect or file argument)
 * something like this:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This program then parses the ABNF and outputs a class that can decode/encode messages according to the ABNF.
 ...
 * </pre>
 * 
 * The generated class also contains Utils.ABNFComponent array that should be used to validate ingoing and outgoing
 * packets.
 * <p>
 * Todo: generate native types instead of generic 'AVP' (requires dictionary); should check ABNF; should make it easy to
 * set origin-host/realm; etc.
 */
public final class ABNFConverter {

  private final static class AVP {
  ...
  public static void main(final String args[]) throws Exception {
    ...
    final ArrayList<AVP> avps = new ArrayList<AVP>();
    for (;;) {
      final String s = br.readLine();
      if (s == null) {
        break;
      ...
      final Matcher m = Pattern.compile("( *)([0-9]*)(\\*)?([0-9]*) *([{\\[<]) *([a-zA-Z-]+) *([}\\]>])").matcher(s);
      if (m.matches()) {
        final AVP avp = new AVP();
