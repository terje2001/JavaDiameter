...
public final class Message {
   ...
   */
  public Message(final MessageHeader header) {
   ...
   */
  public void encode(final byte b[]) {
    final int sz = encodeSize();
    int offset = 0;
   ...
   */
  public static int decodeSize(final byte b[], final int offset) {
    final int v_ml = packunpack.unpack32(b, offset);
    final int v = (v_ml >> 24) & 0xff;
    final int ml = v_ml & 0x00FFFFFF;
    if (v != 1 || ml < 20 || (ml % 4) != 0) {
   ...
   */
  public decode_status decode(final byte b[]) {
   ...
   * @return The result for the decode operation.
   */
  public decode_status decode(final byte b[], int offset, final int bytes) {
    if (bytes < 1) {
    ...
    final int sz = decodeSize(b, offset);
    if ((sz & 3) != 0) {
      return decode_status.garbage;
    }
    if (sz < 20) {
    ...
    final int estimated_avp_count = bytes_left / 16;
    final ArrayList<AVP> new_avps = new ArrayList<AVP>(estimated_avp_count);
    while (bytes_left > 0) {
      if (bytes_left < 8) {
        return decode_status.garbage;
      }
      final int avp_sz = AVP.decodeSize(b, offset, bytes_left);
      if (avp_sz == 0) {
        return decode_status.garbage;
      }
  ...
  public void ensureCapacity(final int minCapacity) {
    avp.ensureCapacity(minCapacity);
  }

  /** Gets the AVP at the specified index (0-based) */
  public AVP get(final int index) {
    return new AVP(avp.get(index));
  }
  ...
  public void add(final int index, final AVP avp) {
    this.avp.add(index, avp);
  }
  ...
  private class AVPIterator implements Iterator<AVP> {
    private final ListIterator<AVP> i;
  ...
  public Iterator<AVP> iterator(final int code) {
    return iterator(code, 0);
  }

  /** Returns an iterator for the AVPs with the specified code and vendor id */
  public Iterator<AVP> iterator(final int code, final int vendor_id) {
   ...
   */
  public void prepareResponse(final Message request) {
   ...
   * @since 0.9.3
   */
  public void prepareAnswer(final Message request) {
   ...
   * </pre>
   */
  public Iterable<AVP> subset(final int code) {
    return subset(code, 0);
  }
   ...
   */
  public Iterable<AVP> subset(final int code, final int vendor_id) {
   ...
   */
  public AVP find(final int code) {
    return find(code, 0);
  }
   ...
   */
  public AVP find(final int code, final int vendor_id) {
    for (final AVP a : avp) {
