 ...
 * <p>
 * The announced vendor-IDs can be used for implementing tweaks in areas not
 * fully specified in the application specification.
 * <p>
 * {@link dk.i1.diameter.ProtocolConstants} contains definitions for some of the standard applications
 * (DIAMETER_APPLICATION_....)
 * <p>
...
public final class Capability {
   ...
   */
  public Capability(final Capability c) {
    supported_vendor = new HashSet<Integer>();
    for (final Integer i : c.supported_vendor) {
      supported_vendor.add(i);
    }
    auth_app = new HashSet<Integer>();
    for (final Integer i : c.auth_app) {
      auth_app.add(i);
    }
    acct_app = new HashSet<Integer>();
    for (final Integer i : c.acct_app) {
      acct_app.add(i);
    }
    auth_vendor = new HashSet<VendorApplication>();
    for (final VendorApplication va : c.auth_vendor) {
      auth_vendor.add(va);
    }
    acct_vendor = new HashSet<VendorApplication>();
    for (final VendorApplication va : c.acct_vendor) {
      acct_vendor.add(va);
    }
  }

  /** Returns if the specified vendor ID is supported */
  public boolean isSupportedVendor(final int vendor_id) {
   ...
   */
  public boolean isAllowedAuthApp(final int app) {
   ...
   * If the application "relay" is listen, then the auth-application is always allowed.
   */
  public boolean isAllowedAcctApp(final int app) {
   ...
   */
  public boolean isAllowedAuthApp(final int vendor_id, final int app) {
   ...
   * Returns if the specified vendor-specific application is an allowed auth-application.
   */
  public boolean isAllowedAcctApp(final int vendor_id, final int app) {
    return acct_vendor.contains(new VendorApplication(vendor_id, app));
  }

  public void addSupportedVendor(final int vendor_id) {
    supported_vendor.add(vendor_id);
  }

  public void addAuthApp(final int app) {
    auth_app.add(app);
  }

  public void addAcctApp(final int app) {
    acct_app.add(app);
  }

  public void addVendorAuthApp(final int vendor_id, final int app) {
    auth_vendor.add(new VendorApplication(vendor_id, app));
  }

  public void addVendorAcctApp(final int vendor_id, final int app) {
   ...
   */
  public static Capability calculateIntersection(final Capability us, final Capability peer) {
    //assumption: we are not a relay
    final Capability c = new Capability();
    for (final Integer vendor_id : peer.supported_vendor) {
      if (us.isSupportedVendor(vendor_id)) {
        c.addSupportedVendor(vendor_id);
      }
    }

    for (final Integer app : peer.auth_app) {
    ...
    for (final Integer app : peer.acct_app) {
      if (app == ProtocolConstants.DIAMETER_APPLICATION_RELAY ||
    ...
    for (final VendorApplication va : peer.auth_vendor) {
    ...
    for (final VendorApplication va : peer.acct_vendor) {
      //relay app is not well-defined for vendor-app
