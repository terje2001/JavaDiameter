...
final public class Utils {
  private Utils() {
  } //stop javadoc from listing this.

  private static boolean contains(final int[] a, final int x) {
    for (final int i : a) {
      if (i == x) {
        return true;
      }
  ...
  private static boolean setMandatory(final AVP a, final int codes[], final int grouped_avp_codes[]) {
    boolean modified = false;
    if (a.vendor_id == 0 && contains(grouped_avp_codes, a.code)) {
      try {
        final AVP_Grouped avp_g = new AVP_Grouped(a);
        final AVP[] avp_g_avps = avp_g.queryAVPs();
        for (final AVP avp_g_avp : avp_g_avps) {
   ...
   * @param grouped_avp_codes Array of AVP codes for AVPs that are grouped and should be examined deeper for other
   *        mandatory AVPs
   * @since 0.9.5
   */
  public static void setMandatory(final Iterable<AVP> avps, final int codes[], final int grouped_avp_codes[]) {
   ...
   * @param codes Array of codes
   */
  public static void setMandatory(final Iterable<AVP> avps, final int codes[]) {
   ...
   * @param codes Array of codes
   */
  public static void setMandatory(final Iterable<AVP> avps, final Collection<Integer> codes) {
    for (final AVP a : avps) {
   ...
   * @param codes Array of codes
   */
  public static void setMandatory(final Message msg, final int codes[]) {
   ...
   * @since 0.9.5
   */
  public static void setMandatory(final Message msg, final int codes[], final int grouped_avp_codes[]) {
   ...
   */
  public static void setMandatory_RFC3588(final Iterable<AVP> avps) {
   ...
   * New behaviour since 0.9.5: Also traverses grouped AVPs mentioned in rfc4006 and handles the M-bit properly.
   */
  public static void setMandatory_RFC3588(final Message msg) {
   ...
   * @since 0.9.2
   */
  public static void setMandatory_RFC4006(final Iterable<AVP> avps) {
   ...
   * @since 0.9.2
   */
  public static void setMandatory_RFC4006(final Message msg) {
   ...
   */
  public static void copyProxyInfo(final Message from, final Message to) {
    for (final AVP avp : from.subset(ProtocolConstants.DI_PROXY_INFO)) {
      to.add(new AVP(avp));
    }
  }
  ...
  static public class ABNFComponent {
    ...
    public ABNFComponent(final boolean fixed_position,
  ...
  public static class CheckABNFFailure {
    ...
    public CheckABNFFailure(final AVP avp, final int result_code, final String error_message) {
      this.failed_avp = avp;
   ...
   */
  public static CheckABNFFailure checkABNF(final Message msg, final ABNFComponent abnf[]) {
    ...
    for (int i = 0; i < abnf.length; i++) {
      final int code = abnf[i].code;
      if (code == -1) {
        continue; //no checks on arbitrary AVPs
      }
      if (abnf[i].min_count == 0 && abnf[i].max_count == -1) {
        continue; //no real limits.
      }
      final int count = msg.count(code);
      if (count < abnf[i].min_count) {
        ...
        int j = 0;
        for (final AVP avp : msg.subset(code)) {
          j++;
      ...
      if (abnf[i].fixed_position) {
        ...
        final int pos = msg.find_first(abnf[i].code);
        if (pos != i) {
    ...
    if (!arbitrary_avps_allowed) {
      ...
      for (final AVP avp : msg.avps()) {
        boolean allowed = false;
        for (final ABNFComponent a : abnf) {
          if (avp.code == a.code) {
            allowed = true;
            break;
          }
