...
public class AVP {
   ...
   * @param payload
   */
  public AVP(final int code, final byte payload[]) {
    this(code, 0, payload);
  }
  ...
  static final int decodeSize(final byte[] b, final int offset, final int bytes) {
    if (bytes < 8) {
      return 0; //garbage
    }
    final int flags_and_length = packunpack.unpack32(b, offset + 4);
    final int flags_ = (flags_and_length >> 24) & 0xff;
    final int length = flags_and_length & 0x00FFFFFF;
    final int padded_length = (length + 3) & ~3;
  ...
  boolean decode(final byte[] b, final int offset, final int bytes) {
    if (bytes < 8) {
      return false;
    }
    int i = 0;
    ...
    i += 4;
    final int flags_and_length = packunpack.unpack32(b, offset + i);
    i += 4;
  ...
  int encode(final byte b[], final int offset) {
    int sz = 4 + 4;
    if (vendor_id != 0) {
      sz += 4;
    }
  ...
  byte[] encode() {
    ...
    final byte[] b = new byte[encodeSize()];
  ...
  void setPayload(final byte[] payload_) {
    setPayload(payload_, 0, payload_.length);
  }

  void setPayload(final byte[] b, final int from, final int count) {
    final byte[] new_payload = new byte[count];
  ...
  public boolean isMandatory() {
    return (flags & avp_flag_mandatory) != 0;
  }

  /** Returns if the private (P) flag is set */
  public boolean isPrivate() {
    return (flags & avp_flag_private) != 0;
  }
   ...
   * @since 0.9.5
   */
  void inline_shallow_replace(final AVP a) {
    payload = a.payload;
    code = a.code;
    flags = a.flags;
    vendor_id = a.vendor_id;
