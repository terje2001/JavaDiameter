...
public abstract class BaseSession implements Session {
   ...
   */
  public int handleRequest(final Message request) {
  ...
  protected int handleASR(final Message msg) {
    if (state_maintained) {
      ...
      final State new_state = State.idle;
  ...
  protected void authSuccessful(final Message msg) {
    ...
    final State old_state = state;
    final State new_state = State.open;
  ...
  public void openSession() throws InvalidStateException {
    ...
    final State new_state = State.pending;
   ...
   */
  public void closeSession(final int termination_cause) {
   ...
   */
  protected void closeSession(final Message msg, final int termination_cause) {
    switch (state) {
      case idle:
        return;
      case pending:
  ...
  private final void sendSTR(final int termination_cause) {
    ...
    } catch (final dk.i1.diameter.node.NotARequestException e) {
      //never happens
    } catch (final dk.i1.diameter.node.NotRoutableException e) {
   ...
   */
  protected void collectSTRInfo(final Message request, final int termination_cause) {
  ...
  protected static final int getResultCode(final Message msg) {
    final AVP avp = msg.find(ProtocolConstants.DI_RESULT_CODE);
    if (avp != null) {
      try {
