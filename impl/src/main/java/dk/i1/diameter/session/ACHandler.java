...
public final class ACHandler {
  ...
  /**
   * The acct-application-id to include in ACRs. If not set, then collectACRInfo() must be overridden to add a
   * vendor-specific-application AVP
   */
  ...
  private final Map<Long, SubSession> subsessions;
  ...
  public long calcNextTimeout() {
    ...
    for (final Map.Entry<Long, SubSession> e : subsessions.entrySet()) {
      final SubSession ss = e.getValue();
  ...
  public long createSubSession() {
    final SubSession ss = new SubSession(subsession_sequencer++);
   ...
   */
  public SubSession subSession(final long subsession_id) {
  ...
  public void startSubSession(final long subsession_id) {
    ...
    final SubSession ss = subSession(subsession_id);
    if (ss == null) {
      return;
    }
    if (ss.start_sent) {
  ...
  public void startSession() {
    final SubSession ss = subSession(0);
    if (ss.start_sent) {
      return;
    }
    sendStart(ss);
   ...
   * Implemented as sendEvent(0,null)
   */
  public void sendEvent(final AVP avps[]) {
    sendEvent(0, avps);
  }
   ...
   */
  public void sendEvent(final long subsession_id) {
   ...
   */
  public void sendEvent(final long subsession_id, final AVP avps[]) {
    final SubSession ss = subSession(0);
    if (ss == null) {
  ...
  private void sendStart(final SubSession ss) {
  ...
  private void sendInterim(final SubSession ss) {
  ...
  private void sendStop(final SubSession ss) {
    sendACR(makeACR(ss, ProtocolConstants.DI_ACCOUNTING_RECORD_TYPE_STOP_RECORD));
  }

  private void sendEvent(final SubSession ss, final AVP avps[]) {
    final Message acr = makeACR(ss, ProtocolConstants.DI_ACCOUNTING_RECORD_TYPE_EVENT_RECORD);
    if (avps != null) {
  ...
  private Message makeACR(final SubSession ss, final int record_type) {
    final Message acr = new Message();
   ...
   * Process an ACA
   */
  public void handleACA(final Message answer) {
    ...
    try {
      ...
      final int record_number = new AVP_Unsigned32(it.next()).queryValue();
      for (final Map.Entry<Long, SubSession> e : subsessions.entrySet()) {
  ...
  private void sendACR(final Message acr) {
    try {
   ...
   */
  public void collectACRInfo(final Message acr, final SubSession ss, final int record_type) {
